...
class Api_model extends CI_Model
{
    ...
    if($type_id == "0") {
        $query = "SELECT * FROM `tbl_partners_availability` WHERE `partner_id`='".$partner_id."' AND `c_date`='".$date."' AND `type_id`='".$type_id."' ORDER BY `id` ASC";

    } else if($type_id == "2") {
        $query = "SELECT * FROM `tbl_partners_availability` WHERE `partner_id`='".$partner_id."' AND `c_date`='".$date."' AND `type_id`='".$type_id."' AND `clinic_id`='".$clinic_id."' ORDER BY `id` ASC";
    } else {
    ...
    public function get_all_data_partners($category_type,$id)
    {
        if($category_type == 8)
        {
            $query="SELECT id,register_type,category,name,email,contact_no,dob,gender,country_code,org_password,password,address,pincode,city,state,country,latitude,longitude,description,profile,address_proof,status_by_admin,is_fill_profile,is_notification,is_verifyotp,social_id,quickblox_name,account_name,account_no,created_at,updated_at,is_clinic,is_online,is_homevisit,(SELECT name FROM tbl_countries WHERE id=tbl_partners.country) as country_name ,(SELECT name FROM tbl_states WHERE id=tbl_partners.state) as state_name ,(SELECT name FROM tbl_cities WHERE id=tbl_partners.city) as city_name,is_update_fees FROM tbl_partners WHERE id='".$id."'";
        }else{
    ...
    public function get_nearest_doctors_list_kunj_new($consultation_type,$latitude,$longitude,$healthcare_id,$healthcare_subid,$appointment_date=NULL,$appointment_time=NULL,$clinic_id=NULL)
    {
        ...
        $this->db->select('tbl_partners.id,tbl_partners.name,tbl_partners.email,tbl_partners.country_code,tbl_partners.contact_no,tbl_partners.category,tbl_partners.is_clinic,tbl_partners.address,tbl_partners.city,tbl_partners.state,tbl_partners.country,tbl_partners.pincode,tbl_partners.latitude,tbl_partners.longitude,tbl_partners.speciality as speciality_id,tbl_partners.profile,tbl_partners.home_visit_main_amt,tbl_partners.online_visit_main_amt,tbl_partners.clinic_visit_main_amt,tbl_partners.quickblox_name,tbl_partners.ug_speciality,tbl_partners_address.clinic_phone_no,tbl_partners_address.longitude,tbl_partners_address.latitude');
        $this->db->select('tbl_services.title as speciality_name');
        $this->db->select('tbl_countries.name as country_name');
        $this->db->select('tbl_states.name as state_name');
        $this->db->select('tbl_cities.name as city_name');
        ...
        if($consultation_type == 0) {
            ...
            $this->db->select('tbl_partners_address.latitude');
            $this->db->select('tbl_partners_address.longitude');
            $this->db->select('tbl_partners_address.clinic_phone_no');
        }else if($consultation_type == 0 || $consultation_type == 2) {
            $this->db->select('tbl_partners_availability.c_date');
            $this->db->select('tbl_partners_availability.c_hour');
            $this->db->select('tbl_partners_availability.clinic_id');
            $this->db->select('tbl_partners_address.latitude');
        ...
        if($consultation_type == 0) {
            ...
            $this->db->select("111.045 * DEGREES(ACOS(COS(RADIANS('".$latitude."')) * COS(RADIANS(tbl_partners_address.latitude)) * COS(RADIANS(tbl_partners_address.longitude) - RADIANS('".$longitude."')) + SIN(RADIANS('".$latitude."')) * SIN(RADIANS(tbl_partners_address.latitude)))) AS `distance_in_km`",false);    
        }
        ...
        if($consultation_type == 0 || $consultation_type == 2)
        {
            ...
            $this->db->join('tbl_partners_address', 'tbl_partners_address.id = tbl_partners_availability.clinic_id','left');
        ...
        if($healthcare_id == 4) {
            $this->db->where('tbl_partners.category',4);
        } else if($healthcare_id == 5) {
            $this->db->where('tbl_partners.category',5);
        } else if($healthcare_id == 6) {
            $this->db->where('tbl_partners.category',6);    
        } else if($healthcare_id == 7) {
            $this->db->where('tbl_partners.category',7);    
        } else {
            $this->db->where('tbl_partners.category',$healthcare_id);
        }
        ...
        if($consultation_type == 2){
            ...
            $this->db->where('tbl_partners.clinic_visit_main_amt >',0); 
        }
        ...
        if($consultation_type == 2){
            ...
            $this->db->having('distance_in_km < 40');
            $this->db->order_by("distance_in_km", "ASC");
        }
    ...
    public function get_address_by_id($id) {
        $query = "SELECT * FROM `tbl_partners_address` WHERE `id`='".$id."' ";
        return $this->db->query($query)->result_array();
    }
    ...
    {
       $query = "SELECT tbl_order.id,tbl_order.category,tbl_order.partner_id,tbl_order.patient_id,name,gender,age,country_code,mobile_no,address,main_amount as fees,appointment_status,customorder_id as appointment_id,request_id,tbl_order.created_at,reinitiate_date,tbl_order.appoitment_date,tbl_order.appoitment_time FROM `tbl_order` LEFT JOIN tbl_notification ON tbl_notification.order_no=tbl_order.id  WHERE tbl_order.category='".$category."' AND tbl_order.partner_id='".$partner_id."' AND  is_reinitiate ='1'";
        return $this->db->query($query)->result_array();       
    }
    public function get_history_list_by_category($category,$patient_id,$appointment_status,$is_followup)
    {
        $this->db->select('tbl_order.id,tbl_order.partner_id,tbl_order.patient_id,tbl_partners.name,consulation_type,customorder_id as appointment_id,appointment_status,is_followup,quickblox_name,tbl_order.created_at,tbl_order.appoitment_date,tbl_order.appoitment_time',false);
        $this->db->from('tbl_order');
        $this->db->join('tbl_partners', 'tbl_partners.id = tbl_order.partner_id', 'left');
        $this->db->where('tbl_order.category',$category);
    ...
    public function get_consultation($category,$partner_id,$consultation_type,$appointment_status,$is_follow,$is_reinitate)
    {
        ...
        $this->db->select('animal_name,tbl_order.id,tbl_order.partner_id,tbl_order.patient_id,customorder_id as appointment_id,consulation_type,appointment_status,tbl_partners.quickblox_name,tbl_order.payment_id,tbl_order.created_at,tbl_order.appoitment_date,tbl_order.appoitment_time',false);
        $this->db->from('tbl_order');
    ...
    public function get_my_consultation($category,$partner_id,$consultation_type,$appointment_status,$is_follow,$is_reinitate) {
        if($category != 5){
            $this->db->select('tbl_order.name as name',false);
        } else {
            $this->db->select('tbl_order.animal_caretaker as name',false);
    ...
    {
        $query = "SELECT tbl_order.id,tbl_order.category,tbl_order.partner_id,patient_id,tbl_order.name,tbl_order.gender,tbl_order.age,tbl_order.height,tbl_order.weight,animal_name,animal_category,animal_caretaker,healthcare_sub_id,tbl_partners.country_code,tbl_partners.contact_no,tbl_order.address,tbl_order.latitude,tbl_order.longitude,tbl_partners.latitude as partner_lat,tbl_partners.longitude as partner_lng,tbl_order.patient_realative,tbl_order.symptoms,tbl_order.main_amount as fees,tbl_order.consulation_type,treatment_days,treatment_per_hours,treatment_hours,111.045 * DEGREES(ACOS(COS(RADIANS(tbl_order.latitude)) * COS(RADIANS(tbl_partners.latitude)) * COS(RADIANS(tbl_partners.longitude) - RADIANS(tbl_order.longitude)) + SIN(RADIANS(tbl_order.latitude)) * SIN(RADIANS(tbl_partners.latitude)))) as distance_in_km,customorder_id as appointment_id,appointment_status,is_followup,tbl_order.payment_id,payment_status,quickblox_name,tbl_order.created_at,tbl_order.reason,tbl_order.appoitment_date,tbl_order.appoitment_time,tbl_order.clinic_id,tbl_partners_address.address as clinic_address FROM `tbl_order` LEFT JOIN tbl_payment_history ON tbl_order.id=tbl_payment_history.order_no LEFT JOIN tbl_partners ON tbl_order.patient_id=tbl_partners.id LEFT JOIN tbl_partners_address ON tbl_order.clinic_id=tbl_partners_address.id WHERE tbl_order.id='".$id."'";
        return $this->db->query($query)->row_array();       
    }
    ...
    public function get_accpted_doctor_list($request_id,$notification_type=NULL,$appointment_date=NULL,$appointment_time=NULL,$clinic_id=NULL,$healthcare_id=NULL,$appointment_type=NULL)
    {
        if(isset($notification_type) && !empty($notification_type)) {
            if(isset($appointment_date) && !empty($appointment_date) && isset($appointment_time) && !empty($appointment_time)) {
                
                if(isset($clinic_id) && !empty($clinic_id)) {

                    $where = "";
                    if($notification_type == "N") {
                        // $where = " AND tbl_partners.home_visit_main_amt > 0 AND tbl_partners.online_visit_main_amt > 0 AND tbl_partners.clinic_visit_main_amt > 0 ";
                        if($healthcare_id == 7 || $healthcare_id == '7') {
                            $where = " AND tbl_partners.home_visit_main_amt > 0 AND tbl_partners.clinic_visit_main_amt > 0 ";
                        } else if($healthcare_id == 6 || $healthcare_id == '6') {
                            $where = " AND tbl_partners.home_visit_main_amt";
                        } else {
                            $where = " AND tbl_partners.home_visit_main_amt > 0 AND tbl_partners.online_visit_main_amt > 0 AND tbl_partners.clinic_visit_main_amt > 0 ";
                        }
                    } 

                    $query="SELECT tbl_notification.id as nid,tbl_notification.category as n_category_id,partener_id,request_id,notification_type,tbl_partners.id,name,address,pincode,city,state,country,latitude,longitude,home_visit_main_amt,online_visit_main_amt,clinic_visit_main_amt,profile,tbl_partners.ug_course,tbl_partners.speciality as speciality_id,(SELECT title FROM tbl_services WHERE id=tbl_partners.speciality) as speciality_name,(SELECT name FROM tbl_countries WHERE id=tbl_partners.country) as country_name,(SELECT name FROM tbl_states WHERE id=tbl_partners.state) as state_name ,(SELECT name FROM tbl_cities WHERE id=tbl_partners.city) as city_name,avg_rate,temp_appointment_data,appointment_date as c_date,appointment_time as c_hour,clinic_id,(SELECT clinic_name FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_name,(SELECT clinic_phone_no FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_phone_no,(SELECT latitude FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_latitude ,(SELECT longitude FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_longitude,(SELECT country FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_country,(SELECT state FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_state,(SELECT city FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_city,(SELECT pincode FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_pincode,(SELECT address FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_address  FROM `tbl_notification` LEFT JOIN tbl_partners ON tbl_notification.partener_id=tbl_partners.id  WHERE tbl_notification.clinic_id='".$clinic_id."' AND tbl_notification.notification_type = '".$notification_type."' AND tbl_notification.appointment_date='".$appointment_date."' AND tbl_notification.appointment_time='".$appointment_time."'  AND tbl_notification.category='".$healthcare_id."' $where  ORDER BY tbl_notification.id DESC";
                } else {
                    ...
                    if($notification_type == "N") {
                        ...
                        if($healthcare_id == 7 || $healthcare_id == '7') {
                            ...
                            $where = " AND tbl_partners.home_visit_main_amt > 0 ";
                        } else {

                            if($appointment_type == 0) {
                                 $where = " AND tbl_partners.online_visit_main_amt > 0  ";
                            }
                            ...
                            if($appointment_type == 2) {
                                 $where = " AND tbl_partners.clinic_visit_main_amt > 0 ";
                            } 
                        }
                    }

                    $query="SELECT tbl_notification.id as nid,tbl_notification.category as n_category_id,partener_id,request_id,notification_type,tbl_partners.id,name,address,pincode,city,state,country,latitude,longitude,home_visit_main_amt,online_visit_main_amt,clinic_visit_main_amt,profile,tbl_partners.ug_course,tbl_partners.speciality as speciality_id,(SELECT title FROM tbl_services WHERE id=tbl_partners.speciality) as speciality_name,(SELECT name FROM tbl_countries WHERE id=tbl_partners.country) as country_name ,(SELECT name FROM tbl_states WHERE id=tbl_partners.state) as state_name ,(SELECT name FROM tbl_cities WHERE id=tbl_partners.city) as city_name,avg_rate,temp_appointment_data,appointment_date as c_date,appointment_time as c_hour,clinic_id,(SELECT clinic_name FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_name,(SELECT clinic_phone_no FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_phone_no,(SELECT latitude FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_latitude ,(SELECT longitude FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_longitude,(SELECT country FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_country,(SELECT state FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_state,(SELECT city FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_city,(SELECT pincode FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_pincode,(SELECT address FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_address  FROM `tbl_notification` LEFT JOIN tbl_partners ON tbl_notification.partener_id=tbl_partners.id  WHERE tbl_notification.notification_type = '".$notification_type."' AND tbl_notification.appointment_date='".$appointment_date."' AND tbl_notification.appointment_time='".$appointment_time."'  AND tbl_notification.category='".$healthcare_id."' $where ORDER BY tbl_notification.id DESC";
                }
            } else {
                ...
                $query="SELECT tbl_notification.id as nid,tbl_notification.category as n_category_id,partener_id,request_id,tbl_partners.id,name,address,pincode,city,state,country,latitude,longitude,home_visit_main_amt,online_visit_main_amt,clinic_visit_main_amt,profile,tbl_partners.ug_course,tbl_partners.speciality as speciality_id,(SELECT title FROM tbl_services WHERE id=tbl_partners.speciality) as speciality_name,(SELECT name FROM tbl_countries WHERE id=tbl_partners.country) as country_name ,(SELECT name FROM tbl_states WHERE id=tbl_partners.state) as state_name ,(SELECT name FROM tbl_cities WHERE id=tbl_partners.city) as city_name,avg_rate,temp_appointment_data,appointment_date as c_date,appointment_time as c_hour,clinic_id,(SELECT clinic_name FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_name,(SELECT clinic_phone_no FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_phone_no,(SELECT latitude FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_latitude ,(SELECT longitude FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_longitude,(SELECT country FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_country,(SELECT state FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_state,(SELECT city FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_city,(SELECT pincode FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_pincode,(SELECT address FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_address  FROM `tbl_notification` LEFT JOIN tbl_partners ON tbl_partners.id=tbl_notification.partener_id  WHERE `request_id`='".$request_id."' AND tbl_notification.notification_type = '".$notification_type."' AND tbl_notification.category='".$healthcare_id."' $where ORDER BY tbl_partners.id ASC";
            }
        } else {
            ...
            $query="SELECT tbl_notification.id as nid,tbl_notification.category as n_category_id,partener_id,request_id,tbl_partners.id,name,address,pincode,city,state,country,latitude,longitude,home_visit_main_amt,online_visit_main_amt,clinic_visit_main_amt,profile,tbl_partners.ug_course,tbl_partners.speciality as speciality_id,(SELECT title FROM tbl_services WHERE id=tbl_partners.speciality) as speciality_name,(SELECT name FROM tbl_countries WHERE id=tbl_partners.country) as country_name ,(SELECT name FROM tbl_states WHERE id=tbl_partners.state) as state_name ,(SELECT name FROM tbl_cities WHERE id=tbl_partners.city) as city_name,avg_rate,temp_appointment_data,appointment_date as c_date,appointment_time as c_hour,clinic_id,(SELECT clinic_name FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_name,(SELECT clinic_phone_no FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_phone_no,(SELECT latitude FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_latitude ,(SELECT longitude FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_longitude,(SELECT country FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_country,(SELECT state FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_state,(SELECT city FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_city,(SELECT pincode FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_pincode,(SELECT address FROM tbl_partners_address WHERE tbl_partners_address.id=tbl_notification.clinic_id) as clinic_address FROM `tbl_notification` LEFT JOIN tbl_partners ON tbl_partners.id=tbl_notification.partener_id  WHERE `request_id`='".$request_id."' AND tbl_notification.category='".$healthcare_id."' AND tbl_notification.req_status = '1' $where ORDER BY tbl_partners.id ASC"; 
        }

        return $this->db->query($query)->result_array(); 
    }

    public function get_accpted_home_doctor_list($request_id)
    {
