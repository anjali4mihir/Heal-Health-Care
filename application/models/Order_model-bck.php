...
class Order_model extends CI_Model {
    ...
    public function get_order_report($category,$partnerId,$order_status){
    	$q=$this->db->query("SELECT * FROM tbl_order as tbo  WHERE tbo.category='".$category."' AND tbo.partner_id='".$partnerId."'AND tbo.order_status='".$order_status."'");
    ...
    public function get_store_by_id($category,$partnerId){
        $q=$this->db->query("SELECT id,address,city,state,country_code,country,pincode,store_name,sign_board,contact_no,gstin,delivery_charge,return_policy,(SELECT name FROM tbl_countries WHERE id=tbl_partners.country) as country_name,(SELECT name FROM tbl_states WHERE id=tbl_partners.state) as state_name,(SELECT name FROM tbl_cities WHERE id=tbl_partners.city) as city_name FROM tbl_partners WHERE category ='".$category."' AND id='".$partnerId."' ");
        return $q->row_array();
    }
