...
class Inventory_model extends CI_Model {
    ...
    public function get_all_medicine_list()
    {
        $this->db->from($this->table_medicine);
        $this->db->where('is_medicine_drug','1');
        $data = $this->db->get();
    ...
    public function make_drug_medicine_query()
    {
        $this->db->select($this->select_column_medicine);
        $this->db->from($this->table_medicine);
        if (isset($_POST["search"]["value"])) {
    ...
    public function make_drug_medicine_datatables()
    {
        $this->make_drug_medicine_query();
        if ($_POST["length"] != -1) {
            $this->db->limit($_POST['length'], $_POST['start']);
        }
        $query = $this->db->get();
    ...
    public function get_drug_medicine_filtered_data()
    {
        $this->make_drug_medicine_query();
        $query = $this->db->get();
        return $query->num_rows();
    ...
    public function get_all_drug_medicine_data()
    {
        $this->db->select("id");
        $this->db->from($this->table_medicine);
        $this->db->where('is_medicine_drug','2');
