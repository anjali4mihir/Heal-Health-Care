...
class Api extends CI_Controller
{
    ...
    public function get_my_availability() {
        ...
        if(count($data_or_not)>0){
            ...
            $this->checkemptyvalue('', $this->get_message_value_from_key($language_id, 'record_not_found'));
            exit;            
        }
    }
    ...
    public function delete_single_my_availability() {
        ...
        if(count($data_or_not)>0){
            ...
            // check entery exist or not
            $get_data = $this->Api_model->get_my_avaiblity_single_day($userid,$current_date);
            if(count($get_data)>0){
