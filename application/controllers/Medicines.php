...
class Medicines extends CI_Controller {
                ...
                if(isset($_POST['submit']) && !empty($_POST['submit']) && $_POST['submit'] == "Delete All") {
                    ...
                    if(isset($_POST['chk_id']) && !empty($_POST['chk_id']) && count($_POST['chk_id'])>0) {
                        $selectedIDs = implode(',',$_POST['chk_id']);
                         ...
                         $this->db->query("DELETE FROM `tbl_store_wise_medicines` WHERE `partner_id` = '" .$partner_id. "' AND `id` IN ($selectedIDs)");
    ...
    public function importdata()
    {
        ...
        if($partner_id > 0)
        {
            //log_message('info', print_r($_REQUEST['records'],true));
            //exit;
            if(isset($_REQUEST['records']) && !empty($_REQUEST['records']))
            {
                foreach($_REQUEST['records'] as $row)
                {
                    ...
                    $expiray_date= ($row['EXPIRAY DATE']);
                    $sdata['expiray_date'] = date('Y-m-d',strtotime($expiray_date));
                    $sdata['created_at'] = date('Y-m-d H:i:s');
                    ...
                    $ddata['expiry_date'] = date('Y-m-d',strtotime($expiray_date));
                    $ddata['batch_no'] = isset($row['BATCH NO']) ? $row['BATCH NO']:"";
