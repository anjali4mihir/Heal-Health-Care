...
class Api extends CI_Controller
{
    ...
    public function get_my_consulatiton()
    {
        ...
        if($is_reinitate == 0){
            ...
            $countAppoitments = $this->Api_model->get_consultation($category_type, $partner_id, $consultation_type, $appointment_status,$is_followup,$is_reinitate);
            $message = "";
            if ($appointment_status == 0) {
    ...
    public function add_my_availability() {
        ...
        $data_or_not = $this->Api_model->check_my_avaiblity_exist_or_not($partner_id,$start_date,$end_date);
        if(count($data_or_not)>0){
            $this->checkemptyvalue($clinic_id, $this->get_message_value_from_key($language_id, 'date_already_exist'));
            exit;
        } else {
            ...
            if(count($available_date_arr)>0) {
                foreach ($available_date_arr as $key => $value) {
                    foreach ($hour_arr as $h_key => $h_value) {
                        ...
                        $userid = $this->Common_model->data_insert('tbl_partners_availability', $tmp_array);
                   }   
                }
            }

            return $this->sendResponse([], $this->get_message_value_from_key($language_id,'my_availability_save_successfully'));
            exit;
        }
    }
    ...
    public function get_my_availability() {
        ...
        // check entery exist or not
        $data_or_not = $this->Api_model->get_my_avaiblity_single_day($userid,$current_date);
        
        if(count($data_or_not)>0){
            return $this->sendResponse($data_or_not, $this->get_message_value_from_key($language_id,'record_found'));
            exit;
        } else {
            $this->checkemptyvalue($clinic_id, $this->get_message_value_from_key($language_id, 'record_not_found'));
            exit;            
        }
    }
    ...
    public function add_single_my_availability() {
        ...
        if(isset($hour_arr) && count($hour_arr) > 0) {
            ...
            foreach ($hour_arr as $h_key => $h_value) {
                ...
                $data_or_not = $this->Api_model->check_my_avaiblity_exit_or_not($userid,$current_date,$h_value);
    ...
    public function delete_single_my_availability() {
        ...
        if(count($data_or_not)>0){
            ...
            return $this->sendResponse([], $this->get_message_value_from_key($language_id,'my_availability_deleted_successfully'));
            exit;
        } else {
            $this->checkemptyvalue('', $this->get_message_value_from_key($language_id, 'Please_try_again'));
            exit; 
        }
    }
}