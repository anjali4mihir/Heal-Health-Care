...
class Api extends CI_Controller
{
    ...
    // public function sendNotDataResponse($response, $message, $status = "0")
    // {
    //     $result = array();
    //     $result['code'] = "200";
    //     $result['status'] = $status;
    ...
    public function get_setting()
    {
        ...
        $appsetting = array(
            ...
            "test_keyID"            => $data['app_setting']['test_key'],
            "test_secrate_key"      => $data['app_setting']['test_secrate_key'],
            "live_keyID"            => $data['app_setting']['live_key'],
            "live_secrate_key"      => $data['app_setting']['live_secrate_key'],
        );
        $finalarray = array(
            "imagepath" => $imagepath,
    ...
    public function login()
    {
        ...
        if (!empty($data) && isset($data['id']) && !empty($data['id'])) {
            ...
            // else
            // {
            //     return $this->sendSuccessErrorResponse("Your status is deactive.Please contact to admin");
            //     exit;
            // }
            //}
            // else if(!empty($data['status_by_admin']) && $data['status_by_admin']==2)
            // {
            //     return $this->sendSuccessErrorResponse("Your account is rejected. Please contact to admin.");
            //     exit;
            // }
            // else
            // {
            //     return $this->sendSuccessErrorResponse("Your account is not Approve. Please contact to admin.");
            //     exit;
            // }
            //}
            ...
            //}
            // else{
            //      return $this->sendSuccessErrorResponse("Your OTP is not Verified");
            //         exit;
            // }
        } else {
    ...
    public function forgot_password()
    {
        ...
        } else if (strtoupper($flag) == 'M' && $mobile_no != '' && $phonecode != '') {
    ...
    public function speciality_list()
    {
        ...
        $data['speciality_list'] = $this->Api_model->get_sub_service_list($category_type);
       ...
       // print_r($data);die;
        ...
        if(count($data) > 0) {
            ...
            $patientnotification_insert = [
                        ...
                        "description"       => "Number of ".$categoryLabel." " .count($data)."  Accepted Your Request. View Details.",
                        "temp_appointment_data"=>$temp_appointment_data,
                        "request_id"        => $random_no,
                        "created_at"        => date('Y-m-d H:i:s')
                    ];
            ...
            $patientnotification_message['type'] = "New Appointment Request Pending";
            $patientnotification_message['title'] = $patientNotificationTitle;
            $patientnotification_message['body'] = "Number of ".$categoryLabel.count($data)."  Accepted Your Request. View Details.";
            ...
            $notification_message['type'] = "New Appointment Request";
            ...
            return $this->sendErrorResponse("Sorry For Your Inconvenience, Currently No Services Can Be Found In Your Area. We Will Reach You Shortly");
        }
    }
    ...
    public function accept_or_reject_reqest()
    {
        ...
        if($status == '1' || $status == 1){
            ...
            if($category_type == 4 || $category_type == 5){
                if($is_reinitate == 1){
                    $Title =  "Accepted Re-Consultation Request";
                }else{
                    $Title =  "Accepted Consultation Request";
                }
                $Message = "Your Online/Home Consultation With ".$senderRecord->name." Has Been Confirmed"; 
            }else{
                if($is_reinitate == 1){
                ...
                $Message = "Your Home Visit Service With Patient- ".$senderRecord->name." Has Been Confirmed. Consult Now"; 
            }
    ...
    public function doctor_details()
    {
        ...
        $customerdata['total_patient_count'] = 0;
        $customerdata['rating'] = array();
    ...
    public function add_address()
    {
        ...
        $this->Common_model->data_insert('tbl_partners_address', $data);

        return $this->sendSuccesResponse("Address Add Successfully");
        exit;
    ...
    public function update_address()
    {
        ...
        $customerdata = $this->Api_model->get_all_address_by_partner($userid, $category_type);
        return $this->sendResponse($customerdata, "Adress Updated Successfully");
        exit;
    }
    ...
    public function medicine_order()
    {
        ...
        $username = 'rzp_test_RzaruHMiBKVQXH';
        $password = 'RSwAdpP3NQ7wR9wbL3aCd3FM';
        $curl = curl_init();
        ...
        if (strtoupper($response->status) == 'CAPTURED' || strtoupper($response->status) == 'AUTHORIZED') {
            ...
            if (count($Medicine) > 0) {
                for ($i = 0; $i < count($Medicine); $i++) {
                    ...
                    $medicine_array = [
                        ...
                        "batch_no"      => $medicine_records['batch_no'],
                        ...
                        "expiray_date"  => $medicine_records['expiray_date'],
            ...
            $notification_insert = [
                ...
                "description"   => "Your Medicine Order-".$customorder_id." Has Been Confirmed",
                "request_id"    => "",
            ];
            $this->notification_entery($notification_insert);
            $patient_notification_insert = [
                ...
                "order_no"      => $lastinsertID,
                "title"         => "New Order For Medicines",
                "description"   => "Your Medicine Order-(Order ID) Has Been Confirmed",
                "request_id"    => "",
            ];
            ...
            if($notification_insert)
            {
                ...
                $notification_message['title'] = "Medicine Order Created";
                $notification_message['body'] = "Your Order Now been Created. View Order Details";
    ...
    public function pathology_order()
    {
        ...
        if (strtoupper($response->status) == 'CAPTURED' || strtoupper($response->status) == 'AUTHORIZED') {
            ...
            if($notification_insert)
            {
                ...
                $notification_message = array();
                $notification_message['type'] = "New Order";
                $notification_message['title'] = "Lab Test Booking Created";
                $notification_message['body'] = "You’re Booking Now been Created. View Order Details";
                $notification_message['order_id'] = $lastinsertID;
                $notification_message['order_status'] = $orderRecord->order_status;
                $notification_message['request_id'] = '';
                $notification_message['patient_id'] = $patient_id;
    ...
    public function history_details()
    {
        ...
        $this->checkuser($patient_id, '8', $language_id);
        $this->userexists($patient_id, $currendeviceid, $authtoken, $language_id);
        $countAppoitments = $this->Api_model->get_history_details_by_id($id, $patient_id);
    ...
    public function book_appoitment()
    {
        ...
        if (strtoupper($response->status) == 'CAPTURED' || strtoupper($response->status) == 'AUTHORIZED') {
            ...
            $notification_insert = [
                ...
                "order_id"      => $customorder_id,
                "order_no"      => $lastinsertID,
                "title"         => "New Appointment Request Confirmed",
                "description"   => $name." has confirmed " .$Keyvalue,
                "request_id"    => "",
            ];
            $this->notification_entery($notification_insert);
            ...
            if($notification_insert)
            {
                ...
                $notification_message = array();
                $notification_message['type'] = "Book Appointment";
                $notification_message['title'] = "New Appointment Request Confirmed";
                $notification_message['body'] = $name." has confirmed  " .$Keyvalue;
                ...
                $notification_patient_message['type'] = "Book Appointment";
                $notification_patient_message['title'] = "New Appointment Request Confirmed";
                $notification_patient_message['body'] = $receiverRecord->name." has confirmed  " .$Keyvalue;
    ...
    public function appoitment_change_status()
    {
        ...
        if ($appointment_status == 1) {
            $message = $appoitmentRecord->partner_name.' has start your consultation';
            $title = 'Consultation Start';
        } else if ($appointment_status == 2) {
            if($category_type == 4 || $category_type == 5){
                $message = "Your Online/Home Consultation With ".$appoitmentRecord->partner_name." Has Been Completed. You Have Period Of 7 days For Follow up.";
                $title = 'Completed Consultation';
            }else{
                $message = "Your Online/Home Service With ".$appoitmentRecord->partner_name." Has Been Completed. You Have Period Of 7 days For Follow up.";
            ...
            if($category_type == 4 || $category_type == 5){
                $message = "Your Online/Home Consultation With ".$appoitmentRecord->partner_name." Has Been Cancelled By ".$appoitmentRecord->partner_name." For ".$reason."";
                $title = 'Cancelled Consultation';
            }else{
                $message = "Your Online/Home Service With ".$appoitmentRecord->partner_name." Has Been Cancelled By ".$appoitmentRecord->partner_name." For ".$reason."";
                $title = 'Cancelled Service';
            }
            
        } else if ($appointment_status == 4) {
            $message = "Your Online/Home Consultation With ".$appoitmentRecord->partner_name." Has Been Cancelled By You  For ".$reason."";
        ...
        $notification_insert = [
            ...
            if($notification_insert)
            {
                ...
                //push_notification_android($device_token['registration_ids'],$notification_message);
                $this->sendResponse(array(),$this->get_message_value_from_key($language_id,$message));
    ...
    public function add_generated_prescription()
    {
        ...
        if($id > 0){
            ...
            // $pdfFilename = "prescription-".$certificate_name.".pdf";
            // $pdfsavePath = $this->config->item('prescription_images_path').$pdfFilename;

            // $this->load->library('Pdf');
            
            // $this->pdf->pdf->SetHTMLHeader('<!doctype html>
            //         <html lang="en">
            //         <head><section class="banner">
            //         </head>
            //         <body>
            //             <div class="logo">
            ...
            //                 <h4>'.$data["doctor_data"]["name"].'</h4>
            //                 <p>'.$data["doctor_data"]["ug_course"].'</p>
            //                 <p>Reg.No - '.$data["doctor_data"]["ug_reg_no"].'</p>
            //             </div>
            //             </section>');

            // $this->pdf->pdf->SetHTMLFooter('<section class="footer">
            // Disclaimer : This prescription is based on the information provided by you in an online consulation and not on any physical Verification. visit a docotor in case of emergency. This Prescription Is valid in india only.
            // </section></body></html>');
            // $this->pdf->pdf->AddPage('P', // L - landscape, P - portrait 
            //             '', '', '', '',
            //             0, // margin_left
            //             0, // margin right
            //            40, // margin top
    ...
    public function sendchat()
    {
        ...
        if($insert)
        {
            ...
            $notification_insert = [
                        ...
                        "title"             => 'New Message',
                        "description"       => $senderRecord->name.' has send you new message by ' .$message,
                        "request_id"        => "",
                        "chat_id"           => $insert,
                    ];
            $this->Common_model->data_insert('tbl_notification', $notification_insert);
            ...
            $notification_message['body'] = $senderRecord->name.' has send you new message by ' .$message;
    ...
    public function my_chat_list()
    {
        ...
        if (count($data) > 0)
        {
            ...
            foreach ($data as $k => $row)
            {
                ...
                $user_details = $this->Api_model->getuserimage($get_image_id);
                $user_name = $this->Api_model->getusername($row->appoitment_id);
                $chatlist[$k] = $row;
                $chatlist[$k]->name = isset($user_name->name) ? $user_name->name:"";
                $chatlist[$k]->profile = isset($user_details->profile) ? $user_details->profile : "";
                ...
                {
                    $chatlist[$k]->profile = $returnpath . $user_details->profile;
                }
            }
    ...
    {
        $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" . $lat1 . "," . $long1 . "&destinations=23.0326487,72.5596985&mode=driving&language=pl-PL&key=AIzaSyC711vkhHG424lDbLWy3ZH0CIgPVDHb8Dc";
        $ch = curl_init();
    ...
    // public function Fortestnotifiation()
    // {
    //     $notification_message = array();
    //     $notification_message['type'] = "Chat";
    //     $notification_message['title'] = 'For Testing ';
    //     $notification_message['body'] = 'Hello';
    //     $this->load->helper('notifications_helper');
    //     push_notification_android('47CA18EE40BC4804EEE70F0B5E6CD1EB293D1DC11330A6AC41B3FE55EB46EEEE',$notification_message);
    // }
