...
class Pathologyservices extends CI_Controller {
    ...
    public function index()
    {
        ...
        if($partner_id >= 1){
            ...
            // delete all start
                if(isset($_POST['submit']) && !empty($_POST['submit']) && $_POST['submit'] == "Delete All") {
                    ...
                    if(isset($_POST['chk_id']) && !empty($_POST['chk_id']) && count($_POST['chk_id'])>0) {
                        ...
                        $this->db->query("DELETE FROM `tbl_test_pathology_wise` WHERE `partner_id` = '" .$partner_id. "' AND `id` IN ($selectedIDs)");
            ...
            $data["services"] = $this->Pathology_model->get_test_list($_SESSION['userid']);
            $this->load->ptemplate('partner/pathologytest/test_list', $data);
        }else{
    ...
    public function add()
    {
        ...
        if($partner_id >= 1){
            if (isset($_REQUEST)) {
                    ...
                    $this->form_validation->set_rules('name', 'Name', 'trim|required|callback_check_test_name_lab_wise');
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        if($this->check_test_name() == 0)
                        {
                            ...
                            $testId = $this->db->get_where('tbl_test_pathology',['name' => strtolower($this->input->post("name"))])->row()->id;
                        }
                        $ddata=array();
                        ...
                        $ddata['is_homesample'] = $this->db->get_where('tbl_partners',['id' => $ddata['partner_id']])->row()->is_homesample;

                        $ddata['status'] = ($this->input->post("status") == "on") ? '1' : '0';
                        
                        $this->load->model("common_model");
                        $ddata['created_at'] = date('Y-m-d H:i:s');
                        $this->common_model->data_insert("tbl_test_pathology_wise", $ddata, TRUE);
                ...
                $this->load->ptemplate('partner/pathologytest/test_add', $data);
        }else{
            redirect('partners/login');
        } 
    }
    ...
    public function edit($id)
    {
        ...
        $data['action'] = "Edit";
        $partner_id = $this->session->userdata('userid');
        $data["service_records"] = $this->Pathology_model->get_test_by_id($id,$partner_id);
        ...
        if($partner_id >= 1) {
            if (isset($_REQUEST)) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    // {
                    //     $config['upload_path'] = './uploads/labservice/';
                    //     $config['allowed_types'] = 'gif|jpg|png|jpeg|svg';
                    //     $this->upload->initialize($config);
                    //     $this->load->library('upload', $config);
                    //     $temp = explode(".", $_FILES["file"]["name"]);
                    //     $newfilename = time() .'_'.rand(11111,99999).'.' . end($temp);
                    //     $uploadpath = $this->config->item('labservice_images_path');
                    ...
                    // }
                        ...
                        $checkslug = $this->Pathology_model->check_slug_edit($ddata['slug'],$_SESSION['userid'],$id);
                    ...
                    $this->common_model->data_update("tbl_test_pathology_wise", $ddata, array("id" => $id));
            ...
            $this->load->model("language_model");
            $data['language'] = $this->language_model->get_language_list();
            $this->load->ptemplate('partner/pathologytest/test_edit', $data);
        } else {
            redirect('partners/login');
        }       
    }
    public function delete()
    {
        ...
        if($partner_id >= 1) {
            ...
            $service = $this->Pathology_model->get_test_by_id($id,$_SESSION['userid']);
            //print_r($service);die;
            if ($service) {
                $this->db->query("DELETE FROM `tbl_test_pathology_wise` WHERE `id` = '" . $service->id . "' AND `partner_id` = '" .$service->partner_id. "'");
                $output = array(
                    'Status'  => 200,
    ...
    public function alldelete($id)
    {
        ...
        if($id==1)
        {
                ...
                $data = array();
                $this->db->query("DELETE FROM `tbl_test_pathology_wise` WHERE  `partner_id` = '" .$partner_id. "'");
                $this->session->set_flashdata("message", '<div class="alert alert-success alert-dismissible" role="alert" id="error">
                                <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                <strong>Success ! </strong>Pathology Test deleted successfully
                              </div>');
                redirect("pathologyservices");
                exit;
        } else {
             $data = array();
                $this->db->query("DELETE FROM `tbl_report_radiology_wise` WHERE  `partner_id` = '" .$partner_id. "'");
                $this->session->set_flashdata("message", '<div class="alert alert-success alert-dismissible" role="alert" id="error">
                                <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                <strong>Success ! </strong>Radiology Services deleted successfully
                              </div>');
    ...
    {
        $responsedata = $this->Pathology_model->get_all_test_list_by_search($this->input->post('search'));
        $output = '';
        $output = array(
        ...
        if($partner_id > 0)
        {
            if(isset($_REQUEST['records']) && !empty($_REQUEST['records']))
            {
                ...
                foreach($_REQUEST['records'] as $row)
                {
                    ...
                    {
                        $Id = $this->common_model->data_insert("tbl_test_pathology",$data, TRUE);
                        $testId =  $Id;
                    }
                    ...
                    $this->db->select('id');
                    $this->db->from('tbl_test_pathology_wise');
                    ...
                    if(isset($recordexists) && !empty($recordexists) && !empty($recordexists['id']))
                    {
                        ...
                        $this->db->update('tbl_test_pathology_wise',$ddata,array('id'=>$recordexists['id']));
                    }
                    else
                    {
                        $ddata['created_at'] = date('Y-m-d H:i:s');
    ...
    public function check_test_name()
    {
        ...
        if(!empty($name)) {
            $check_medicine_exist = $this->Pathology_model->check_test_exist_name($name);
    ...
    public function check_test_exist_or_not()
    {
        $name = strtolower($this->input->post("name"));
        if(!empty($name)) {
            $check_medicine_exist = $this->Pathology_model->check_test_exist_or_not($name,$_SESSION['userid']);
            if($check_medicine_exist>0){
                 echo "false";
                 exit;
            } else {
    ...
    public function check_edit_test_exist_or_not()
    {
        ...
        //print_r($id);die;
        if(!empty($name)) {
            $check_medicine_exist = $this->Pathology_model->check_edit_labservice_exist_or_not($name,$_SESSION['userid'],$id);
            //print_r($check_medicine_exist);die;
            if($check_medicine_exist > 0){
                 echo "false";
                 exit;
    ...
    {
        $this->db->select('*');
        $this->db->where('LOWER(name)',strtolower($this->input->post('name')));
        $this->db->where('partner_id',$this->session->userdata('userid'));
        if($this->db->get('tbl_test_pathology_wise')->num_rows() > 0){
    ...
    public function check_edit_test_name_lab_wise()
    {
        ...
        if($this->db->get('tbl_test_pathology_wise')->num_rows() > 0){
            $this->form_validation->set_message(__FUNCTION__, 'Service is Already Exists');
            return false;
        }
        else
        {
    ...
    public function show_all_test()
    {
        ...
        {
        $fetch_data = $this->Pathology_model->make_test_datatables($_SESSION['userid']);
        ...
        foreach ($fetch_data as $key => $row) {
            $edit_url = base_url() . 'pathologyservices/edit/'.$row['id'];
            ...
            $sub_array[] = '<input type="checkbox" class="js-switch tgl_checkbox" data-status="status" data-table="tbl_test_pathology_wise" data-idfield="id" data-id="'.$row['id'].'" id=cb_'.$row['id'].'  '.(($row['status'] == "1") ? 'checked' : "").' >';
        ...
        $output = array(
            ...
            "recordsTotal" => $this->Pathology_model->get_all_test_data(),
            "recordsFiltered" => $this->Pathology_model->get_test_filtered_data($_SESSION['userid']),
            "data" => $data,
            "status" => 200,
        );
        }else{
