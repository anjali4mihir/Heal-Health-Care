...
class Inventory extends CI_Controller {
    ...
    public function importdata() { 
            ...
            /*echo "<br/>Postdata---<pre>";
            echo "<br/>file----".$file;
            print_r($_POST);
            echo "<br/>Base URL---".base_url("inventory/importdata");
            exit;*/

            $str = '<script src="https://code.jquery.com/jquery-3.3.1.min.js"></script><form name="frm_import" id="frm_import" enctype="multipart/form-data" method="post" action="'.base_url("inventory/importdata").'">';
            ...
            while(($filesop = fgetcsv($handle, 1000, ",")) !== false) {
                ...
                if($this->input->post('type') == 'pharmacy') {
                    $z='<input type="hidden" name="row_num" values="'.$row.'" />';
                    $medicineId = '';
                    if($c<>0) {
                        ...
                        if(isset($_POST['row_num']) && ($_POST['row_num'] > 0)) {
                            ...
                            if($filesop[0]!='' && $filesop[1]!='' && $filesop[2]!='' && $filesop[3]!='') {
                                ...
                                if($check_medicine_exist <= 0) {
                                    ...
                                    $this->db->insert('tbl_medicine',$sdata);
                            ...
                            if($filesop[0]!='' && $filesop[1]!='' && $filesop[2]!='' && $filesop[3]!='' &&($row > 1))  {
                                //print_r($filesop[2]);die;
                                $check_medicine_exist = $this->Inventory_model->check_medicine_exist_or_not(strtolower($filesop[0]), strtolower($filesop[2]), strtolower($filesop[3]),strtolower($filesop[1]));
                                //print_r($check_medicine_exist);die;
                                if($check_medicine_exist <= 0) {
                        ...
                        if($row%50 == 0) { 
                            echo "YES IN row%50 <br />";
                            echo $str;
                            ?>
                            <script type="text/javascript">$("#frm_import").submit();</script>
                        <?php }
                    }
                    $c = $c + 1;
