            ...
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                    <div class="full_width network-btn-main">
                        <a href="<?= base_url().'medicines/add' ?>" class="add-post-btn add-company-btn float-right"><i class="fa fa-plus"></i> Add</a>
                        <a href="<?= base_url().'medicines/csv_structure_download/1'?>" class="add-post-btn add-company-btn float-left">CSV file structure</a>
                        <a href="#" class="add-post-btn add-company-btn float-left" onclick="importData()"> Import</a>
                        <a href="javascript:void(0);" onclick="confirm_delete('<?= base_url() . 'Pathologyservices/alldelete/1'; ?>')"  title="Delete All" class="add-post-btn add-company-btn float-left">Delete All</a>
                    </div>  
                    <div class="full_width all_tables table-responsive rating-table load-table">                
                        ...
                        <div class="alert alert-success alert-dismissible" role="alert" id="myElem" style="display:none">
                            <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button><strong>Success !</strong> Medicine activated Successfully.
                        </div>
                        <div class="alert alert-danger alert-dismissible" role="alert" id="myElemNo"  style="display:none">
                            <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button><strong>Success !</strong> Medicine deactivated Successfully. 
                        </div>
                        ...
                        <br/>
                            <table id="example2321" class="example table table-striped table-bordered table-data-load" style="width:100%">
                                <thead>
                                    <tr>
                                        <th scope="col">
                                        ...
                                        <th scope="col">MRP</th>
                                        <?php /* <th scope="col">Sale Price</th> */?>
                                        <th scope="col">GST Amt.</th>
                                        <th scope="col">Discount</th>
                                        <th scope="col">Total</th>
                                        <th scope="col">EXP</th>
                                        <th scope="col">Stock(Pack)</th>
...
$('#open-popup').magnificPopup({
    items: [
      {
        src: '<?= base_url().'assets/img/Untitled-3.png' ?>',
        title: 'csv file structure for import medicines'
      },
    ],
    gallery: {
      enabled: true
...
insure_PotentialImport_js = {
        ...
        getConfigChunk: function (results, parser) {
            ...
            jQuery.ajax({
                url: '<?= base_url("medicines/importdata"); ?>',
                type: 'post',
                data: {records: data},
                async: false,
                dataType: 'json',
    ...
    $('#example2321').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                url: "<?=base_url('medicines/show_all_medicines');?>",
                type: "POST",
                data: {
                    token: hash
                }
...
$(function () {
        $("body").on("change", ".tgl_checkbox", function () {
            ...
            .done(function (msg) {
                //  alert(msg);
                if (msg == '1') {
                    PNOTY("Medicine Unblock successfully", 'success');
                    
                } else if (msg == '0') {
                    PNOTY("Medicine Block successfully", 'danger');
                }
            });
        });
});
