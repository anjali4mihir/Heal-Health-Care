            ...
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                    <div class="full_width network-btn-main">
                        <a href="<?= base_url().'radiologyservices/add_test'?>" class="add-post-btn add-company-btn float-right"><i class="fa fa-plus"></i>Add</a>
                        <a href="<?= base_url().'radiologyservices/csv_structure_download/4'?>" class="add-post-btn add-company-btn float-left">CSV file structure</a>
                        <a href="#" class="add-post-btn add-company-btn float-left" onclick="importData()"> Import</a>
                        <a href="javascript:void(0);" onclick="confirm_delete('<?= base_url() . 'Pathologyservices/alldelete/3'; ?>')"  title="Delete All" class="add-post-btn add-company-btn float-left">Delete All</a> <?php /*  */ ?>
                    </div>
                    <div class="full_width all_tables table-responsive rating-table load-table">                
                        ...
                        <div class="alert alert-success alert-dismissible" role="alert" id="myElem" style="display:none">
                            ...
                            <strong>Success !</strong> Test activated Successfully.
                        </div>
                        <div class="alert alert-danger alert-dismissible" role="alert" id="myElemNo"  style="display:none">
                            <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                            <strong>Success !</strong> Test deactivated Successfully. 
                        </div> 
                        ...
                        <table id="table_radio" class="example table table-striped table-bordered table-data-load" style="width:100%">
                            <thead>
                                <tr>
                                    <th scope="col">
                                    ...
                                    <!-- <th scope="col">Modality/Group/Category</th> -->
                            ...
                            <!--<tbody>
                                <?php if (count($services) > 0) {
                                    ...
                                    foreach ($services as $category) {
                                        $edit_url = base_url() . 'radiologyservices/edit_test/' . $category->id;
                                        $delete_url = base_url() . 'radiologyservices/delete_test/' . $category->id;
                                        ?>  
                                        <tr>
                                            ...
                                            <td><?php echo number_format((float)$category->sale_price, 2, '.', ''); ?></td>
                                             ...
                                             <td><input type="checkbox" class="js-switch tgl_checkbox"  data-table="tbl_report_radiology_wise" data-status="status" data-idfield="id"  data-id="<?php echo $category->id; ?>" id='cb_<?php echo $category->id; ?>'  <?php echo ($category->status == '1') ? "checked" : ""; ?> /></td>
                                            <td>
...
$('#open-popup').magnificPopup({
    items: [
      {
        src: '<?= base_url().'assets/img/Untitled-5.png' ?>',
...
insure_PotentialImport_js = {
        ...
        getConfigChunk: function (results, parser) {
            ...
            jQuery.ajax({
                url: '<?= base_url("radiologyservices/importdata"); ?>',
                type: 'post',
                data: {records: data},
                async: false,
                dataType: 'json',
    ...
    $('#table_radio').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                url: "<?=base_url('radiologyservices/show_all_test');?>",
                type: "POST",
                data: {
                    token: hash
                }
    ...
    swal({
        ...
        function(isConfirm) {
            if (isConfirm) {
                ...
                $.ajax({
                    url: '<?= base_url().'radiologyservices/delete_test'?>',
                    method: "POST",
                    dataType: "JSON",
                    cache: false,
                    data: {
...
$(function () {
        $("body").on("change", ".tgl_checkbox", function () {
            ...
            .done(function (msg) {
                //  alert(msg);
                if (msg == '1') {
                    PNOTY("data Unblock successfully", 'success');
                    
                } else if (msg == '0') {
                    PNOTY("data Block successfully", 'danger');
                }
            });
        });
});
