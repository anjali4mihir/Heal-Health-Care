            ...
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                    <div class="full_width network-btn-main">
                        <a href="<?= base_url().'security_features/add' ?>" class="add-post-btn add-company-btn float-right"><i class="fa fa-plus"></i>Add</a>
                    </div>  
                    <div class="full_width all_tables table-responsive rating-table load-table">                
                        <?php
                        if (isset($error)) {echo $error;}echo $this->session->flashdata("message");?>
                        <div class="alert alert-success alert-dismissible" role="alert" id="myElem" style="display:none">
                            <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                            <strong>Success !</strong> Feature activated successfully.
                        </div>
                        <div class="alert alert-danger alert-dismissible" role="alert" id="myElemNo"  style="display:none">
                            <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                            <strong>Success !</strong> Feature deactivated successfully. 
                        </div>
                        ...
                        <table id="example23" class="example table table-striped table-bordered table-data-load" style="width:100%">
                            ...
                            <?php
                                if (count($testimonials) > 0) {
                                    $returnpath = $this->config->item('security_images_uploaded_path');
                                    $rowno = 1;
                                    foreach ($testimonials as $category) {
                                        $edit_url = base_url() . 'security_features/edit/' . $category->id;
                                        $delete_url = base_url() . 'security_features/delete/' . $category->id;?>  
                                        <tr>
                                             <td>
                                        <input name="chk_id[]" type="checkbox" class="custBox" value="<?php echo $category->id; ?>"/>
                                    </td>
                                            <td><?php echo $rowno; ?></td>
                                            <td><img src="<?php echo $returnpath.$category->file; ?>" class="round" alt="image" height="50px" width="50px"></td>
                                            <td><?php echo $category->language_name; ?></td>
                                            <td><?php echo $category->name; ?></td>
                                            <td><?php echo $category->details; ?></td>
                                            <td><input type="checkbox" class="js-switch tgl_checkbox"  data-table="tbl_security_features" data-status="status" data-idfield="id"  data-id="<?php echo $category->id; ?>" id='cb_<?php echo $category->id; ?>'  <?php echo ($category->status == '1') ? "checked" : ""; ?> /></td>
                                            <td>
                                                <a href="<?php echo $edit_url; ?>" data-toggle="tooltip"  data-placement="left" title="Edit" class=""><i class="fa fa-edit btn btn-warning btn-sm"></i></a>
                                                 <a href="javascript:void(0);" onclick="confirm_delete('<?php echo $delete_url; ?>')" data-toggle="tooltip"  data-placement="left" title="Delete" class=""><i class="fa fa-trash-o btn btn-danger btn-sm"></i></a>
                                            </td>
                                    </tr>
                                    <?php $rowno++; } } ?>
                            </tbody>
