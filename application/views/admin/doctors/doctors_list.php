            ...
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                    <div class="full_width network-btn-main">
                        <a href="<?= base_url().'doctors/add' ?>" class="add-post-btn add-company-btn float-right"><i class="fa fa-plus"></i>Add</a>
                    </div>  
                        ...
                        <table id="example23" class="example table table-striped table-bordered table-data-load" style="width:100%">
                            ...
                            <?php
                                if (count($doctors) > 0) {
                                    ...
                                    $rowno = 1 ;
                                    foreach ($doctors as $category) {
                                        $edit_url = base_url() . 'doctors/edit/' . $category->id;
                                        $delete_url = base_url() . 'doctors/delete/' . $category->id;?>  
                                        <tr>
                                            <td>
                                            ...
                                            <td>
                                                <?php if($returnpath != '' && $category->file != '' && file_exists($returnpath.$category->file)) { ?> 
                                                <img src="<?php echo $returnpath.$category->file; ?>" class="round" alt="image" height="50px" width="50px">
                                                <?php } ?>
                                            </td>
                                            <td><?php echo $category->name; ?></td>
                                            <td><?php echo $category->position; ?></td>
                                            <td><?php echo $category->details; ?></td>
                                            <td><input type="checkbox" class="js-switch tgl_checkbox"  data-table="tbl_doctors" data-status="status" data-idfield="id"  data-id="<?php echo $category->id; ?>" id='cb_<?php echo $category->id; ?>'  <?php echo ($category->status == '1') ? "checked" : ""; ?> /></td>
